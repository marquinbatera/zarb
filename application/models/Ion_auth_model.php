...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function getDadosEditCliente($id)
	{
		$this->db->select('id, tipo_cliente, username, cpf_cnpj, informacao_tributaria,
							inscricao_estadual, data_nascimento, sexo, phone, telefone_celular,
							users.id_user_endereco, email, cep, TB_ESTADOS.id_estado as estado, 
							TB_ESTADOS.estado as desc_estado, cidade, bairro, 
							TB_LOGRADOUROS.id_logradouro, logradouro, numero, complemento, referencia');
		$this->db->from('users');
		$this->db->join('TB_CLIENTES_ENDERECO', 'users.id_user_endereco = TB_CLIENTES_ENDERECO.id_user_endereco');
		$this->db->join('TB_LOGRADOUROS', 'TB_CLIENTES_ENDERECO.id_logradouro = TB_LOGRADOUROS.id_logradouro');
		$this->db->join('TB_BAIRROS', 'TB_LOGRADOUROS.id_bairro = TB_BAIRROS.id_bairro');
		$this->db->join('TB_CIDADES', 'TB_BAIRROS.id_cidade = TB_CIDADES.id_cidade');
		$this->db->join('TB_ESTADOS', 'TB_CIDADES.id_estado = TB_ESTADOS.id_estado');
